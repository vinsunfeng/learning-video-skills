...
### 为什么把这条放在正文第一句，而不是只写在 description 里

**因为写在 description 里被实测证明拦不住。** 2026-08-08 用
`scripts/trigger_test.py` 量过（Hermes + 20 条正负样例）：

| 版本 | 该触发 | 不该触发 | 「这视频讲了啥」 |
|---|---|---|---|
| description 只在末尾写一句「不适用」 | 9/9 | 4/11 | 误触发 |
| description 把排除项列成 6 条显眼清单 | 9/9 | 10/11 | **仍误触发，重复 3 次 3/3** |

**3/3 是稳定的，不是噪声**（同轮正对照 3/3、负对照 0/3，仪器可靠）。
理由每次都类似：「video-distill 正是做 YouTube 摘要的」——
**模型看到「视频 → 结构化」就点着了，而排除句压不过这个正信号。**
